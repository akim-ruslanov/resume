...
def crawl_profile(session, base_url, profile_url, post_limit):
    ...
    while n_scraped_posts < post_limit:
        try:
            ...
            posts = profile_bs.find('div', id=posts_id).section.article.div
        except Exception:
            posts_id = 'structured_composer_async_container'
            # posts = profile_bs.find('div', id=posts_id).div.div.contents
            print(profile_bs.find('div', id=posts_id))

        posts_urls = [a['href'] for a in profile_bs.find_all('a', attrs = {"aria-label":"Open Story"})] 
